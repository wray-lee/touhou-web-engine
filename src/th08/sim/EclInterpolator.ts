 ...
 */
export const INTERP_HERMITE = 7;

/** The position registers. Driving one makes the frame tail convert to velocity. */
export const REG_POS_X = 0x273a;
export const REG_POS_Y = 0x273b;
export const REG_POS_Z = 0x273c;
 ...
 */
export function easeProgress(easing: number, progress: number): number {
  switch (easing) {
    case 1:
...
/** `InterpolateLinear` (`EclDependencies.cpp:318-328`). */
function interpolateLinear(world: InterpWorld, slot: InterpSlot, t: number): void {
  const start = world.resolve(slot.p0);
  const end = world.resolve(slot.p1);
  world.write(slot.variable, (end - start) * t + start);
}

/** `InterpolateHermite` (`EclDependencies.cpp:332-357`), the Catmull-Rom basis. */
function interpolateHermite(world: InterpWorld, slot: InterpSlot, t: number): void {
  const p0 = world.resolve(slot.p0);
  const p1 = world.resolve(slot.p1);
  const p2 = world.resolve(slot.p2);
  const p3 = world.resolve(slot.p3);
  const w0 = (t - 1.0) * (t - 1.0) * (2.0 * t + 1.0);
  const w1 = t * t * (3.0 - 2.0 * t);
  const w2 = (1.0 - t) * (1.0 - t) * t;
  const w3 = (t - 1.0) * t * t;
  world.write(slot.variable, w0 * p0 + w1 * p1 + w2 * p2 + w3 * p3);
}

export class EclInterpolator {
  readonly slots: InterpSlot[] = Array.from({ length: INTERP_SLOTS }, blankSlot);

  /** `EnemyManager` zeroes the context with the enemy. */
  reset(): void {
    for (const slot of this.slots) Object.assign(slot, blankSlot());
  }

  /** A slot is running while its callback pointer is set. */
  get activeCount(): number {
    let n = 0;
    for (const slot of this.slots) if (slot.live) n++;
    return n;
  }
  ...
  install(
    ...
    let slot: InterpSlot | null = null;
    for (const candidate of this.slots) {
      if (candidate.live && candidate.variable !== variable) continue;
      slot = candidate;
      break;
    }
  ...
  step(world: InterpWorld): boolean {
    let drivesPosition = false;
    for (const slot of this.slots) {
      if (!slot.live) continue;
      slot.timer++;
      if (slot.timer >= slot.duration) slot.timer = slot.duration;
      // A zero duration would divide to NaN and poison the register it writes.
      const t = slot.duration > 0 ? easeProgress(slot.easing, slot.timer / slot.duration) : 1;
      if (slot.callback === INTERP_HERMITE) interpolateHermite(world, slot, t);
      else interpolateLinear(world, slot, t);
      if (slot.timer >= slot.duration) slot.live = false;
      const id = Math.trunc(slot.variable);
      if (id === REG_POS_X || id === REG_POS_Y || id === REG_POS_Z) drivesPosition = true;
    }
